...
void EXTI0_1_IRQHandler(void) {
    ...
    //HAL_GPIO_TogglePin(GPIOC, GPIO_PIN_9);			// Once again Toggle green LED 

    
    //HAL_GPIO_TogglePin(GPIOC, GPIO_PIN_8);		// Once again Toggle orange LED 
		
    
    HAL_GPIO_EXTI_IRQHandler(GPIO_PIN_0);				// Clear the EXTI 
}
/**
...
int main(void)
{
  ...
  /* USER CODE END Init */
	...
	HAL_GPIO_WritePin(GPIOC, GPIO_PIN_9, GPIO_PIN_SET);//sets pin 9 high
  ...
  /* USER CODE END 2 */
	...
	EXTI->IMR = 0x0001;  //SET EXTI0 TO ALLOW INTERRUPTS
	EXTI->RTSR = 0x0001;  // Setting EXTI input line 0 to have rising-edge trigger
															
	SYSCFG->EXTICR[0] |= SYSCFG_EXTICR1_EXTI0_PA;   // Configure multiplexer to route PA0 to the EXTI input line 0
															 
	// Enable EXTI0_1_IRQn for EXTI line 0 interuppt by passing defined name
	NVIC_EnableIRQ(EXTI0_1_IRQn);
	
	// Setting interrupt to 1 
